...
class Distributor():
    ...
    def log(self, entryNumber):
        ...
        for i in [self.Log.loc[len(self.Log.index) - 1][0]] + self.Log.loc[len(self.Log.index) - 1][3]:
            self.entryBalancePerPerson(self.EqualDistributor(self.Log.loc[len(self.Log.index) - 1][2], len(self.Log.loc[len(self.Log.index) - 1][3])))
